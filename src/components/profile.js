...
const Profile = () => {
  ...
  const handleCancelRSVP = async (eventId) => {
    try {
      ...
      // Update user data after cancellation
      const userData = await getCurrentUser();
      setUser(userData);
    } catch (err) {
  ...
  const handleUpdateNotificationPrefs = async (field, value) => {
    try {
      ...
      // Update user data after preference change
      const userData = await getCurrentUser();
      setUser(userData);
    } catch (err) {
      ...
      <div className="tab-content">
                ...
                {user.performanceSlots.map(performance => (
                  <div key={`${performance.event._id}-${performance.slot._id}`} className="performance-card">
                    <div className="performance-info">
                      ...
                      <p className="venue">
                        <i className="bi bi-geo-alt"></i> {performance.event.venue}
                      </p>
                      <p className="date">
                        <i className="bi bi-calendar"></i> {new Date(performance.event.date).toLocaleDateString()}
                      </p>
                      <p className="time">
                        <i className="bi bi-clock"></i> {new Date(performance.slot.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </p>
                      <p className="slot-info">
                        <i className="bi bi-mic"></i> Slot #{performance.slot.order}
                      </p>
                    </div>
                    <div className="performance-actions">
                      <Link to={`/event/${performance.event._id}`} className="btn btn-primary">
                        View Event
                      </Link>
                      <button 
                        className="btn btn-danger" 
                        onClick={() => handleCancelPerformance(performance.event._id, performance.slot._id)}
                      >
                        Cancel Performance
                      </button>
                    </div>
        ...
        {activeTab === 'hosted' && user.userType === 'host' && (
          <div className="hosted-tab">
            ...
            {user.eventsHosting && user.eventsHosting.length > 0 ? (
              <div className="event-cards">
                  ...
                  <div key={event._id} className="card event-card">
                    ...
                    <div className="card-body">
                      ...
                      <p className="card-text attendees">
                        <i className="bi bi-people"></i> {event.attendeeCount || 0} attendees
                      </p>
                      <div className="card-actions">
                        <Link to={`/event/${event._id}`} className="btn btn-primary">
                          View Details
        ...
        {/* Notification Settings Tab (for comedians) */}
          ...
          <div className="notifications-tab">
            ...
            <div className="notification-settings">
              ...
              <div className="form-check form-switch">
                ...
                />
                <label className="form-check-label" htmlFor="availableSlotAlerts">
                  New Available Performance Slots
                </label>
              </div>
              ...
              <div className="form-check form-switch">
                ...
                />
                <label className="form-check-label" htmlFor="upcomingPerformanceReminders">
